...
use parity_bn::{arith::U256, AffineG1, Fr};
...
use super::{
    converter::g1_to_bytes, error::PlonkError, kzg, utility, PlonkProof, ALPHA, BETA, GAMMA, U,
    ZETA,
};
#[derive(Debug)]
...
pub(crate) fn verify_plonk_algebraic(
    ...
    // Original: Fr::from_slice(&n.to_bytes_be())
    let n = Fr::from_slice(&utility::u256_to_bytes_be(&n))
        .map_err(|e| PlonkError::GeneralError(Error::Field(e)))?;
    let zeta_power_n = zeta.pow(n);
    let zh_zeta = zeta_power_n - one;

    // Compute L₁(ζ) = (ζⁿ - 1) / (n * (ζ - 1))
    ...
    for i in 0..vk.commitment_constraint_indexes.len() {
        ...
        hash_to_field.reset();
        // Original: Fr::from_bytes_be_mod_order(&hash_bts).map_err(|_| Error::FailedToGetFrFromRandomBytes)?
        let hashed_cmt = utility::fr_from_bytes_be_mod_order(&hash_bts);
    ...
    // Original: Fr::from_slice(&n_plus_two.to_bytes_be())
    let n_plus_two = Fr::from_slice(&utility::u256_to_bytes_be(&n_plus_two))
        .map_err(|e| PlonkError::GeneralError(Error::Field(e)))?;
    ...
    // Original: AffineG1::msm(&points, &scalars)
    let linearized_polynomial_digest = utility::affine_g1_msm(&points, &scalars);

    // Build digests_to_fold directly instead of using vec![AffineG1::default(); ...]
    // (parity-bn's AffineG1 doesn't implement Default)
    let mut digests_to_fold = Vec::with_capacity(vk.qcp.len() + 6);
    digests_to_fold.push(linearized_polynomial_digest); // [0]
    digests_to_fold.push(proof.lro[0]); // [1]
    digests_to_fold.push(proof.lro[1]); // [2]
    digests_to_fold.push(proof.lro[2]); // [3]
    digests_to_fold.push(vk.s[0]); // [4]
    digests_to_fold.push(vk.s[1]); // [5]
    digests_to_fold.extend_from_slice(&vk.qcp); // [6..]

    // Original: [vec![const_lin], proof.batched_proof.claimed_values.clone()].concat()
    ...
    let (folded_proof, folded_digest) = kzg::fold_proof(
        ...
        &batch_opening_proof,
        &zeta,
        // Original: Some(zu.into_u256().to_bytes_be().to_vec())
        Some(utility::u256_to_bytes_be(&zu.into_u256()).to_vec()),
        &mut fs,
    )?;
...
fn bind_public_data(
    ...
    for public_input in public_inputs.iter() {
        // Original: public_input.into_u256().to_bytes_be()
        transcript.bind(challenge, &utility::u256_to_bytes_be(&public_input.into_u256()))?;
    }
...
fn derive_randomness(
    ...
    // Original: Fr::from_bytes_be_mod_order(b.as_slice())
    let x = utility::fr_from_bytes_be_mod_order(b.as_slice());
    Ok(x)
}
