...
pub(crate) fn verify_plonk_algebraic(
    ...
    debug(format!(
        "    transcript_init + challenges: {} M cycles",
    ...
    debug(format!("    pi_computation: {} M cycles", (current_cycles() - c) / 1024 / 1024));
    ...
    debug(format!(
        "    linearization_coefficients: {} M cycles",
        (current_cycles() - c) / 1024 / 1024
    ));
    ...
    let c = current_cycles();
    debug(format!("    linearized_poly_msm ({} points):", points.len()));
    let mut linearized_polynomial_digest: Option<AffineG1> = None;
    for (i, (p, s)) in points.iter().zip(scalars.iter()).enumerate() {
        if s.is_zero() {
            continue;
        }
        ...
        linearized_polynomial_digest =
            linearized_polynomial_digest.map(|acc| acc + term).or(Some(term));
    ...
    let linearized_polynomial_digest = linearized_polynomial_digest.unwrap();
    debug(format!(
        "    linearized_poly_msm total: {} M cycles",
        (current_cycles() - c) / 1024 / 1024
    ));
    ...
    debug(format!("    fold_proof: {} M cycles", (current_cycles() - c) / 1024 / 1024));
    ...
    debug(format!("    derive_u: {} M cycles", (current_cycles() - c) / 1024 / 1024));
    ...
    debug(format!(
        "    batch_verify_multi_points: {} M cycles",
        (current_cycles() - c) / 1024 / 1024
    ));
